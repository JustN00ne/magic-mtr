(function () {
  ...
  function bootstrap() {
    try {
      document.documentElement?.classList?.add("jme-collapse-route-chips");
    } catch (ignored) {
      // Ignore DOM/classList failures.
    }
  ...
  function drawMapOverlay() {
    ...
    if (state.showBaseRails) {
      drawRails(ctx, pxPerBlock, preparedRails, {
        color: BASE_RAIL_COLOR,
        alpha: state.showDetails ? 0.72 : 0.92,
        widthScale: state.showDetails ? 1.55 : 1.25,
      });
    }

    if (state.showDetails) {
      drawRails(ctx, pxPerBlock, preparedRails, { alpha: 0.95, widthScale: 1 });
  ...
  function drawRails(ctx, pxPerBlock, preparedRails, options) {
    const opts = options && typeof options === "object" ? options : {};
    const widthScale = Number.isFinite(opts.widthScale) ? opts.widthScale : 1;
    const lineWidth = clamp(pxPerBlock * 0.22 * widthScale, 1.2, 6.8);
    const alpha = Number.isFinite(opts.alpha) ? clamp(opts.alpha, 0, 1) : 0.95;
    const colorOverride = typeof opts.color === "string" && opts.color.trim() ? opts.color.trim() : "";
  ...
  function drawVehicles(ctx, pxPerBlock) {
    ...
    state.vehicles.forEach(vehicle => {
      ...
      if (state.respectRouteFilters && routeId && !isRouteIdVisibleByMode(routeId)) {
        return;
      }
  ...
  function loadLocalOverlaySettings() {
    ...
    if (showBaseRails === "true" || showBaseRails === "false") {
      state.showBaseRails = showBaseRails === "true";
    }

    const showDetails = readLocalStorage("jme_show_details");
    if (showDetails === "true" || showDetails === "false") {
      state.showDetails = showDetails === "true";
    }

    const showSignals = readLocalStorage("jme_show_signals");
    if (showSignals === "true" || showSignals === "false") {
      state.showSignals = showSignals === "true";
    }

    const showVehicles = readLocalStorage("jme_show_vehicles");
    if (showVehicles === "true" || showVehicles === "false") {
      state.showVehicles = showVehicles === "true";
    }

    const respectRouteFilters = readLocalStorage("jme_respect_route_filters");
    if (respectRouteFilters === "true" || respectRouteFilters === "false") {
      state.respectRouteFilters = respectRouteFilters === "true";
    }
  }

  function saveLocalOverlaySettings() {
    writeLocalStorage("jme_show_base_rails", String(!!state.showBaseRails));
    writeLocalStorage("jme_show_details", String(!!state.showDetails));
    writeLocalStorage("jme_show_signals", String(!!state.showSignals));
    writeLocalStorage("jme_show_vehicles", String(!!state.showVehicles));
    writeLocalStorage("jme_respect_route_filters", String(!!state.respectRouteFilters));
  }
  ...
  function initMenu() {
    ...
    panel.innerHTML = `
      ...
      <div class="jme-menu-section">
        <div class="jme-menu-section-title">Overlay Layers</div>
        <div class="jme-menu-col">
          <label class="jme-menu-toggle">
            <span class="jme-menu-toggle-text">Rails (orange)</span>
            <input type="checkbox" data-jme-local="showBaseRails" />
          </label>

          <label class="jme-menu-toggle">
            <span class="jme-menu-toggle-text">Details (speed, signals, trains)</span>
            <input type="checkbox" data-jme-local="showDetails" />
          </label>

          <label class="jme-menu-toggle" data-jme-details-only>
            <span class="jme-menu-toggle-text">Signals</span>
            <input type="checkbox" data-jme-local="showSignals" />
          </label>

          <label class="jme-menu-toggle" data-jme-details-only>
            <span class="jme-menu-toggle-text">Vehicles</span>
            <input type="checkbox" data-jme-local="showVehicles" />
          </label>

          <label class="jme-menu-toggle">
            <span class="jme-menu-toggle-text">Respect route filters</span>
            <input type="checkbox" data-jme-local="respectRouteFilters" />
          </label>
        </div>
    ...
    panel.querySelectorAll("[data-jme-local]").forEach(control => {
      if (!control) {
        return;
      }
      control.addEventListener("change", event => {
        event.stopPropagation();
        const key = control.getAttribute("data-jme-local");
        if (!key || !(key in state)) {
          return;
        }
        state[key] = !!control.checked;
        saveLocalOverlaySettings();
        updateMenuState();
      });
    });
  ...
  function updateMenuState() {
    ...
    state.menuPanel.querySelectorAll("[data-jme-local]").forEach(control => {
      const key = control && control.getAttribute ? control.getAttribute("data-jme-local") : "";
      if (!key || !(key in state)) {
        return;
      }
    ...
    state.menuPanel.querySelectorAll("[data-jme-details-only]").forEach(row => {
      row.hidden = !state.showDetails;
    });
  }
