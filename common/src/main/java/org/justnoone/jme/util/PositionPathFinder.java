...
/**
 * Pathfinding helper for a {@link Position}-to-{@link SavedRailBase} path.
 *
 * <p>This is similar to {@link SavedRailPathFinder}, but allows starting from an arbitrary node
 * (for example, a signal right before a station throat) to support mid-route rerouting.
 */
public final class PositionPathFinder {

    private PositionPathFinder() {
    }
    ...
    public static ObjectArrayList<PathData> findPath(
        ...
        if (data == null || transportMode == null || startPosition == null || endSavedRail == null) {
        ...
        final List<Position> ends = new ArrayList<>(2);
        if (endPos1 != null) {
            ends.add(endPos1);
        }
        if (endPos2 != null && !endPos2.equals(endPos1)) {
            ends.add(endPos2);
        }
        ...
        for (int i = 0; i < ends.size() && System.currentTimeMillis() < deadlineMillis; i++) {
            final Position endPosition = ends.get(i);
            if (endPosition == null) {
                continue;
            }
            ...
            final long perAttemptBudget = Math.max(10, remainingMillis / Math.max(1, ends.size() - i));
    ...
    private static PathCandidate tryFindPath(
        ...
        final FlexiblePathFinder pathFinder = new FlexiblePathFinder(data, transportMode, startPosition, endPosition);
        ...
        for (int i = 0; i < maxCalls && System.currentTimeMillis() < attemptDeadlineMillis; i++) {
            final PathCandidate candidate = pathFinder.tickToCandidate(endSavedRail, stopIndex, cruisingAltitude);
            if (candidate == null) {
                continue;
            }
    ...
    private static void padPositions(
        ...
        final Position edgePosition = positions.get(positions.size() - 1);
        ...
        if (!savedRail.containsPos(edgePosition)) {
            ...
            final Position adjacent = positions.get(positions.size() - 2);
    ...
    private static final class FlexiblePathFinder extends PathFinder<PositionAndAngle> {
        ...
        private PathCandidate tickToCandidate(
            ...
            // Ensure the path includes the "full" end saved rail, matching MTR behavior.
            padPositions(positions, endSavedRail, positionsToRail);
